...
The session polls the local API every 30 s and logs one warning when tailscaled's current endpoint for the remote is inside the session networks. The rig cannot reproduce the loop, because tailscaled runs outside the rig's network namespace, so the warning is the check that finds a regression on a client.
