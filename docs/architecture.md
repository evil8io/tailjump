...
5. `_session start` waits up to 60 s for the state file with status `up` or for the unit to fail, and prints the result. A signal during that wait cancels the connect. `tj connect` sends SIGINT to `sudo` instead of a kill signal, and sudo relays it to the root copy. The root copy stops the unit and waits up to 25 s until the unit is inactive. It then prints `connect cancelled; the session is stopped` and exits 130. A signal after the session came up changes nothing: the session stays up.
