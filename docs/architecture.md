...
3. The client dials `<bind-address>:<port>` with its certificate, ALPN `tj/3`, TLS 1.3, and a verify function that accepts only the helper certificate with the pinned fingerprint. The helper requires a client certificate and accepts only the pinned client fingerprint, and it refuses every handshake once a connection exists.
...
The plan JSON: `{"remote":…,"addr":…,"user":…,"networks":[…],"dns":{"mode":…,"servers":[…],"domains":[…]},"helper_arch":…,"transport":"auto|quic|ssh","quic_ports":"7443-7452","bandwidth_up":0,"bandwidth_down":0,"protocols":"tcp,udp,icmp"}`. The bandwidths are bytes per second, zero for BBR. An empty `protocols` means all three.

The state file `/run/tj/session.json` (0644): `{"remote":…,"addr":…,"user":…,"networks":[…],"dns":{…},"started_at":…,"pid":…,"status":"starting|up|stopping","transport":"quic|ssh","quic_port":…,"fallback":…,"protocols":"tcp,udp,icmp"}`. `tj status` reads it without root and prints `Protocols:`.
