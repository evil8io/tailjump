...
3. The client opens every stream. The first byte of a stream is its kind: `0` control, `1` TCP, `2` UDP, `3` probe, `4` ICMP echo. A probe stream gets the status byte `0` and a close; the QUIC transport uses it right after its handshake.
...
8. Control: exactly one control stream, opened first. The helper writes one JSON line: `{"version":"…","goos":"linux","goarch":"…","hostname":"…","pid":…}`. The client can write the line `quit\n`. The helper then closes its QUIC listener and exits. The helper also exits when the transport closes. The client can write the line `icmp\n`, and the helper answers `icmp raw|ping|none [<low>-<high>]\n` with the echo socket it can open and the `ping_group_range` value; `tj doctor` reports it. The client can write the line `unlink\n`, and the helper removes its own file; there is no reply, because the helper processes the control verbs in order. An extra lane's helper receives none of these verbs: the client uses an extra lane for flows, for the liveness wait, and for `quit` only. The other control verbs are the QUIC negotiation below.
...
3. The client dials `<bind-address>:<port>` with its certificate, ALPN `tj/4`, TLS 1.3, and a verify function that accepts only the helper certificate with the pinned fingerprint. The helper requires a client certificate and accepts only the pinned client fingerprint, and it refuses every handshake once a connection exists.
...
Cost of the fallback, measured in spike 9 on a client on 2026-09-12: with a bulk download in the session, a DNS query takes up to 795 ms on the SSH transport over the relayed path and up to 355 ms over the direct path, against 74 ms and 60 ms on QUIC, because the one TCP connection with the 4 MiB yamux window queues every flow behind the bulk flow. `docs/spikes/09-tj-against-sshuttle.md` has the tables.
...
* `test/e2e/compare.sh` is the tj against sshuttle comparison of spike 9, `task e2e:compare`. It runs on the client, not in the rig, because the rig cannot run the real direct path, one tunnel at a time: tj on QUIC, tj on the SSH transport, and sshuttle with the flags of the Evil8 `connect` task plus `--dns`. Per session it measures a time-boxed download with a DNS query, a TCP connect, and a disco ping every 5 s, then 20 TCP connects, 10 DNS queries, and the system resolver. It reads the remote from `test/e2e/target.env` or from the file in `TJ_TEST_ENV`, and the knobs are `TJ_TEST_SESSIONS`, `TJ_TEST_CYCLES`, `TJ_TEST_DL_SECONDS`, `TJ_TEST_SSHUTTLE_NETS`, and `TJ_TEST_SSHUTTLE_BUF`. sshuttle needs a NOPASSWD sudo rule for its firewall helper. The results are in `docs/spikes/09-tj-against-sshuttle.md`.
